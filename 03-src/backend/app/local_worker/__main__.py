"""本地 worker 主入口：``python -m app.local_worker``。

独立进程，复用 backend venv。**不调用 init_db()**（schema 归云端管），
只复用 ``app.core.database.async_session`` 直连云端 PostgreSQL。
...
async def _run_loop() -> None:
    from app.local_worker.claims import claim_next_job, reclaim_own_host, release_lease
    from app.local_worker.runner import _heartbeat, mark_failed_guarded, process_job
