...
复用云端 ``_run_resume`` 的断点续跑逻辑（pending/capturing 分支会重新采集），
...
async def process_job(job_id: UUID) -> None:
    ...
    崩溃在采集中途（status=capturing）先回退 pending 让 _run_resume 重新采集；
    其余状态直接走 _run_resume 的断点续跑各分支，无需重写 pipeline 序列。
    """
    async with async_session() as db:
        job = await db.get(IngestJob, job_id)
        if job and job.status == "capturing":
            from app.s1_ingest.orchestrator import _force_set_status

            await _force_set_status(db, job_id, "pending")

    from app.s1_ingest.orchestrator import _run_resume
...
async def mark_failed_guarded(job_id: UUID, message: str) -> None:
    ...
    """
    try:
        from app.s1_ingest.orchestrator import transition
