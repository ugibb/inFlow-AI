...
CSDN is standard SSR HTML with mild anti-bot measures.  trafilatura extracts
content reliably via the generic cascade.
"""
...
class CsdnAdapter(AbstractAdapter):
    platform = "csdn"
    version = "1.0.0"

    def can_handle(self, url: str) -> bool:
        return bool(_CSDN_URL_RE.search(url))
    ...
    async def fetch(
        ...
        *,
        ...
        logger.debug("CsdnAdapter.fetch: %s", url[:100])

        try:
            from app.s1_ingest.fetchers import parser_service
            result = await parser_service._fetch_generic(url, "csdn")
        except Exception as exc:
            raise AdapterError(
                f"CSDN fetch failed: {exc}",
