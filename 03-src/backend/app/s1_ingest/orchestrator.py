...
async def _peek_url_title(url: str) -> str | None:
    ...
    import httpx
    from app.s1_ingest.fetchers import extract_url_from_text, parser_service
...
async def ingest_url(
    ...
    *,
    ...
    """
    from app.s1_ingest.fetchers import extract_url_from_text
    ...
    async with async_session() as db:
        try:
            ...
            if getattr(raw_capture.raw, "content_type", None) == "audio":
                ...
                from app.core.shared.storage.conventions import ingest_media_path
...
async def _run_upload_capture(
    *,
    ...
    from app.core.database import async_session
    from app.s1_ingest.adapters.upload import UploadAdapter
...
async def _run_resume(*, job_id: UUID) -> None:
    ...
    async with async_session() as db:
        ...
        if job.status == "normalizing":
            if not job.raw_file_path:
                return
            await _force_set_status(db, job_id, "captured")
            await _route_pipeline(
                db,
                job_id=job_id,
                article_id=article_id,
        ...
        if job.status == "parsing":
            ...
            if job.asr_file_path:
                await _force_set_status(db, job_id, "transcribed")
                await run_chapters(
                    db,
                    job_id=job_id,
                ...
                if not job.raw_file_path:
                    ...
                    return
                asr_txt = parse_asr_txt_path(job.raw_file_path, job_id)
                await _force_set_status(db, job_id, "normalized")
                await run_chapters(
                    db,
                    job_id=job_id,
                    raw_file_path=job.raw_file_path,
        ...
        if job.status == "composing":
            if not job.parsed_file_path or not job.raw_file_path or not article_id:
                return
            await _force_set_status(db, job_id, "parsed")
            asr_for_card = job.asr_file_path
            if content_type != "audio":
                from app.core.shared.storage.conventions import parse_asr_txt_path
                asr_for_card = parse_asr_txt_path(job.raw_file_path, job_id)
        ...
        if job.status == "indexing":
            if not job.parsed_file_path or not job.raw_file_path or not article_id:
                ...
                return
            await _force_set_status(db, job_id, "composed")
            await run_index(
                db,
                job_id=job_id,
                article_id=article_id,
...
async def _force_set_status(db: AsyncSession, job_id: UUID, status: str) -> None:
