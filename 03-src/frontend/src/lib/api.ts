...
class ApiClient {
  ...
  // ── Ingest Pipeline ─────────────────────────────────────────
  async ingestUrl(url: string): Promise<{ job_id: string | null; status: string; article_id?: string; message?: string }> {
  ...
  async ingestUpload(file: File): Promise<{ job_id: string; status: string }> {
    const formData = new FormData();
    formData.append('file', file);
    ...
    const res = await fetch(`${this.baseUrl}/api/ingest/upload`, {
      method: 'POST',
      headers,
      body: formData,
    });
  ...
  async ingestText(text: string, title?: string): Promise<{ job_id: string; status: string }> {
  ...
  async getIngestJob(jobId: string): Promise<import('./types').IngestJob> {
    return this.request(`/api/ingest/jobs/${jobId}`);
  }

  async getActiveIngestJobs(): Promise<import('./types').IngestJob[]> {
    return this.request('/api/ingest/jobs');
  }

  async getJobByArticleId(articleId: string): Promise<import('./types').IngestJob | null> {
    const jobs: import('./types').IngestJob[] = await this.request(
      `/api/ingest/jobs?article_id=${encodeURIComponent(articleId)}`
    );
    return jobs[0] ?? null;
  }

  async getJobTranscript(jobId: string): Promise<import('./types').JobTranscript> {
    return this.request(`/api/ingest/jobs/${jobId}/transcript`);
  }

  async getJobLogs(jobId: string, step?: string): Promise<{ logs: StepLogEntry[] }> {
    const qs = step ? `?step=${encodeURIComponent(step)}` : '';
    return this.request(`/api/ingest/jobs/${jobId}/logs${qs}`);
  }

  async cancelIngestJob(jobId: string): Promise<void> {
    return this.request<void>(`/api/ingest/jobs/${jobId}`, { method: 'DELETE' });
  }

  async retryIngestJob(jobId: string, fromStep: string): Promise<void> {
    return this.request<void>(`/api/ingest/jobs/${jobId}/retry`, {
      method: 'POST',
      body: JSON.stringify({ from_step: fromStep }),
    });
  }

  async getArticleTranscript(articleId: string): Promise<import('./types').JobTranscript> {
    return this.request(`/api/articles/${articleId}/transcript`);
  }

  async getArticleChapters(articleId: string): Promise<import('./types').ArticleChaptersResponse> {
  ...
  async getArticleDeepReadHtml(articleId: string): Promise<string> {
    ...
    if (!res.ok) {
      const error = await res.json().catch(() => ({ detail: res.statusText }));
      const msg = typeof error.detail === 'string' ? error.detail : 'Deep read not available';
      throw new Error(msg);
    }
    return res.text();
  }

  async captureArticleDeepReadScreenshot(articleId: string, html: string): Promise<Blob> {
    const url = `${this.baseUrl}/api/articles/${articleId}/deep-read/screenshot`;
    const token = getToken();
    ...
    if (!res.ok) {
      const error = await res.json().catch(() => ({ detail: res.statusText }));
      const msg = typeof error.detail === 'string' ? error.detail : 'Screenshot failed';
      throw new Error(msg);
    }
    return res.blob();
  }

  async regenerateArticleAI(articleId: string): Promise<{ status: string; article_id: string }> {
  ...
  async wechatGetStatus(): Promise<{
    ...
    return this.request('/api/wechat/status');
  }
