...
parser_service):
    ...
    python -m inflow_server.extensions.wechat.bot
...
from inflow_core.core.models import User, WechatAccount
from inflow_core.core.utils.logger import configure_third_party_loggers, get_logger, setup_logging
    ...
    async def add_article(
        ...
        if r.status_code == 401:
            return False, (
                ...
                "云端：./stop-server.sh && ./start-server.sh --restart。"
            ), None, None
        if r.status_code == 503:
            return False, (
                "❌ 添加失败：无法连接本地后端（可能被系统代理拦截）。"
                "请执行 ./stop-server.sh && ./start-server.sh 重启服务后重试。"
            ), None, None
        try:
...
class AccountWorker:
    ...
    async def _touch_context_token(self, sender: str, ctx: str) -> None:
        ...
        from inflow_core.core.models.wechat_callback import WechatCallbackQueue
    ...
    async def _handle(self, client: httpx.AsyncClient, acct: WechatAccount, msg: dict):
        ...
        if url:
            ...
            if ok and job_id:
                ...
                from inflow_core.core.pipeline.pipeline_log import log_wechat_url_submitted
    ...
    async def _send_image(
        ...
        *,
        ...
        from inflow_server.extensions.wechat.ilink_media import upload_image_item
        ...
        if sent and job_id and push_label:
            from inflow_core.core.pipeline.pipeline_log import log_job_event
    ...
    async def _process_ready_callbacks(self, img_client: httpx.AsyncClient) -> None:
        ...
        for row in rows:
            ...
            from inflow_core.core.pipeline.pipeline_log import log_job_event, resolve_phase_label, resolve_phase_task_name
            ...
            try:
                ...
                if not Path(png_path).is_file() or Path(png_path).stat().st_size == 0:
                    ...
                    from inflow_core.compose.card_renderer import render_card_png_for_job
...
def main():
    ...
    # 与 backend 共用 04-log/backend/日期.log，便于 ./start-server.sh --logs 一处查看
    setup_logging(
