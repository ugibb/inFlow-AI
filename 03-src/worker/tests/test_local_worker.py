...
from inflow_core.core.pipeline.pipeline_steps import _ext_step_done
...
def test_compose_and_continue_upload_failure_stops_before_index():
    import inflow_core.ingest.orchestrator as orch
...
def test_upload_png_skipped_when_sftp_disabled():
    ...
    from inflow_worker.runner import _upload_card_png_via_sftp

    with patch(
        "inflow_worker.runner.worker_settings",
...
def test_mark_failed_guarded_skips_non_owner():
    ...
    from inflow_worker.runner import mark_failed_guarded

    cm = _job_cm(status="parsing", processing_host="other-host")
    with (
        patch("inflow_worker.runner.async_session", return_value=cm),
        patch("inflow_worker.runner.worker_settings", SimpleNamespace(worker_host="my-host-1")),
        patch("inflow_core.ingest.orchestrator.transition", new=AsyncMock()) as trans,
    ):
