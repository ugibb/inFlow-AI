...
```
2026-06-23 20:37:52,512 [trove.auto_backfill] INFO: Auto-backfill background task started
```
...
- 仅 `main.py` 的 `trove.auto_backfill` 和 `wechat/bot.py` 的 `basicConfig` 配置了 handler
- 其余 logger 默认 `propagate=True`，root logger 在 uvicorn 下级别偏高
- **INFO 级业务日志被静默丢弃**

实证：`orchestrator.py` 中有完整 ingest 链路日志（`ingest_url queued`、`Parse step complete` 等），在 `04-log/` 中**搜不到任何一条**；`trove.*` 业务行仅约 30 行 / 5748 行。

### 2.4 命名不一致

| 写法 | 示例 | 数量 |
|------|------|------|
| `trove.xxx` | `trove.ingest.orchestrator` | 多数业务模块 |
...
模块名显示时去掉 `trove.` 前缀（参考 `_PodcastMapFormatter` 逻辑，根名改为 `trove`）。
...
| 原实现 | Trove 适配 |
|--------|------------|
| `_ROOT_LOGGER_NAME = "podcastmap"` | `_ROOT_LOGGER_NAME = "trove"` |
| `setup_logger(log_dir="log")` | `setup_logging(log_dir=settings.log_dir)` |
| `_PodcastMapFormatter` | `_TroveFormatter`（去掉 `trove.` 前缀） |
...
| `app/main.py` | lifespan 开头调用 `setup_logging()`；删除手写 `trove.auto_backfill` handler；`print` 改 `logger.info` |
...
已使用 `logging.getLogger("trove.xxx")` 的模块**无需改 import**，统一根 logger 接管后即可落盘。

使用 `getLogger(__name__)` 的模块（如 `app.core.shared.ai_service`）第一期仍可通过 propagate 到 `trove` 父级输出；第二期统一改为 `get_logger()`。
